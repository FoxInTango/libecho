...
TARGET_NAME_EXT =
#    安装位置
INSTALL_PATH    = /usr/local/libecho
...
install :
	rm -rf /usr/local/include/libecho
	mkdir /usr/local/include/libecho
	cp  -rf ./include/* /usr/local/include/libecho
	cp  -rf ./lib/libcpp.so /usr/local/lib
uninstall : 
	rm -rf /usr/local/include/libecho
	rm -rf /usr/local/lib/libecho.so
